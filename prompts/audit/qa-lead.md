...
**Inputs:** Requirements, Risk  |  Optional: Historical QA Data  |  Context: Project QA Context  |  Preconditions: QA Team Available
...
**Escalation Conditions:** Critical Quality Risk
...
- ورودی‌های الزامی موجود و معتبر باشند (`Quality Criteria`).
...
- خروجی‌ها و شواهد ثبت شده باشند؛ معیار پذیرش `Quality Criteria` برآورده شده باشد.
...
- پلن را به‌صورت فایل Markdown در پوشه‌ی `audits/` ذخیره کن؛ الگوی نام پیشنهادی: `audits/qa-lead-execution-plan.md` (در صورت تعارض یا چند نسخه، پسوند تاریخ/نسخه اضافه کن).
