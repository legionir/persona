...
## ۱) Identity
- **نقش:** Finance Manager (ناظر)
- **مأموریت:** کنترل Financial Health
...
**Inputs:** Budget, Expenses  |  Optional: Revenue Data  |  Context: Financial Context  |  Preconditions: Budget Defined
...
**Escalation Conditions:** Budget Overrun

### STEP 2 — Track  [GENERIC]
...
### STEP 3 — Forecast  [GENERIC]

**Objective:** اجرای گام «Forecast» با حفظ Scope و بدون تغییر خارج از اختیار.
...
- ورودی‌های الزامی موجود و معتبر باشند (`Budget Criteria`).
...
- خروجی‌ها و شواهد ثبت شده باشند؛ معیار پذیرش `Budget Criteria` برآورده شده باشد.
...
## ۸) خروجی و تحویل
- خروجی ممیزی: Financial Reports
- Handoff: Sponsor, Board
...
- پلن را به‌صورت فایل Markdown در پوشه‌ی `audits/` ذخیره کن؛ الگوی نام پیشنهادی: `audits/finance-manager-execution-plan.md` (در صورت تعارض یا چند نسخه، پسوند تاریخ/نسخه اضافه کن).
